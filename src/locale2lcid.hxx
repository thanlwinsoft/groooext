...
// This code should be consistent with the OOo isolang.cxx definitions
namespace org { namespace sil { namespace graphite {

    struct IsoLangEntry
    {
        unsigned short mnLang;
        const sal_Char maLangStr[4];
        const sal_Char maCountry[3];
    };
    ...
    public:
        Locale2Lang() : mSeedPosition(128)
        {
            ...
            for (int i = 0; i < maxIndex; i++)
            {
                ...
                if (mLangLookup[a][b])
                {
                    ...
                    while (old[len]) len++;
                    len += 2;
                    mLangLookup[a][b] = new const IsoLangEntry * [len];
                ...
                {
                    mLangLookup[a][b] = new const IsoLangEntry * [2];
        ...
        unsigned short getMsId(::rtl::OUString language, ::rtl::OUString country, ::rtl::OUString region) const
        {
            size_t a = 'e' - 'a';
            size_t b = 'n' - 'a';
            unsigned short langId = 0;
            int i = 0;
            switch (language.getLength())
            {
                case 2:
                {
                    a = language.getStr()[0] - 'a';
                    b = language.getStr()[1] - 'a';
                    if (mLangLookup[a][b])
                    {
                        while (mLangLookup[a][b][i])
                        {
                            ...
                            if (country.compareToAscii(mLangLookup[a][b][i]->maCountry) == 0)
                            {
                ...
                case 3:
                {
                    ...
                    if (mLangLookup[a][b])
                    {
                        while (mLangLookup[a][b][i])
                        {
                            if (mLangLookup[a][b][i]->maLangStr[2] != language.getStr()[2])
                            {
                                ++i;
                                continue;
                            }
